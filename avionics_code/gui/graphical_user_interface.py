...
class GUI:
    ...
    def __init__(self):
        ...
        self.screen = pygame.display.set_mode((DASHBOARD_SIZE * 1.9, DASHBOARD_SIZE))
        ...
        # drawing surfaces
        D_S = DASHBOARD_SIZE
        self.active_drawing = [0]*6
        self.surf_u_i = pygame.Surface((D_S * 1.9, D_S), pygame.SRCALPHA)
        self.surf_status = pygame.Surface((D_S * 1.9, D_S), pygame.SRCALPHA)
        self.surf_map = pygame.Surface((D_S * 1.2, D_S), pygame.SRCALPHA)
        self.surf_mission = pygame.Surface((D_S * 1.2, D_S), pygame.SRCALPHA)
        self.surf_flight = pygame.Surface((D_S * 1.6, D_S), pygame.SRCALPHA)
        self.surf_path = pygame.Surface((D_S * 1.2, D_S), pygame.SRCALPHA)
    ...
    def display_update(self):
        ...
        self.screen.blit(self.surf_u_i, (0, 0))
        self.screen.blit(self.surf_status, (0, 0))
        self.screen.blit(self.surf_map, (0, 0))
        self.screen.blit(self.surf_mission, (0, 0))
        self.screen.blit(self.surf_path, (0, 0))
        self.screen.blit(self.surf_flight, (0, 0))

    def update_u_i(self):
        """updates control menu"""

        self.active_drawing[0] = 1
        D_S = DASHBOARD_SIZE
        self.surf_u_i = pygame.Surface((D_S * 1.9, D_S), pygame.SRCALPHA)
        ...
        self.display_update()
        self.active_drawing[0] = 0

    def update_system_status(self):
        """updates system status"""

        self.active_drawing[1] = 1
        D_S = DASHBOARD_SIZE
        self.surf_status = pygame.Surface((D_S * 1.9, D_S), pygame.SRCALPHA)
        ...
        self.display_update()
        self.active_drawing[1] = 0

    def update_map(self):
        """updates map profile and mission state"""

        self.active_drawing[2] = 1
        D_S = DASHBOARD_SIZE
        self.surf_map = pygame.Surface((D_S * 1.2, D_S), pygame.SRCALPHA)
        ...
        self.display_update()
        self.active_drawing[2] = 0

    def update_mission(self):
        """updates mission profile and mission state"""

        D_S = DASHBOARD_SIZE
        self.active_drawing[3] = 1
        self.surf_mission = pygame.Surface((D_S * 1.2, D_S), pygame.SRCALPHA)
        ...
        self.display_update()
        self.active_drawing[3] = 0

    def update_flight(self):
        """updates flight profile"""

        self.active_drawing[4] = 1
        D_S = DASHBOARD_SIZE
        self.surf_flight = pygame.Surface((D_S * 1.6, D_S), pygame.SRCALPHA)
        ...
        self.display_update()
        self.active_drawing[4] = 0

    def update_path(self):
        """updates current path"""

        self.active_drawing[5] = 1
        D_S = DASHBOARD_SIZE
        self.surf_path = pygame.Surface((D_S * 1.2, D_S), pygame.SRCALPHA)
        ...
        self.display_update()
        self.active_drawing[5] = 0

    def draw_control_menu(self):
        """draws control menu"""

        surface = self.surf_u_i
        ...
        for key in self.buttons:
            B = self.buttons[key]
            pygame.draw.circle(surface, B[0], (D_S * B[1], B[2] * D_S / 15), B[3] * WAYPOINT_SIZE)
        ...
        pygame.draw.rect(surface, (0, 0, 0), label_pos, width=1)
        surface.blit(label, label_pos)

        # draw line separating two side of the menu
        D_S = DASHBOARD_SIZE
        pygame.draw.line(surface, (0, 0, 0), (D_S*1.2, 0), (D_S*1.2, D_S))
        pygame.draw.line(surface, (0, 0, 0), (D_S*1.4, 0), (D_S*1.4, D_S))
        pygame.draw.line(surface, (0, 0, 0), (D_S*1.6, 0), (D_S*1.6, D_S))

    def draw_system_status(self):
        """draws system_status"""
        surface = self.surf_status

        def draw_status(element, name, states, y):
            if element is not None:
                for index, state in enumerate(states):
                    if element.status == index:
                        d_f.draw_centered_text(surface, self.font, name + " " + state, (1.75, y))

        draw_status(g_v.sc, "Fake Server:", ["disconnected", "connecting...", "connected"], 0.5)
        draw_status(g_v.rf, "Pixhawk:", ["disconnected", "connecting...", "connected"], 1.0)
        draw_status(g_v.mp, "M-profile:", ["missing", "retrieving...", "retrieved"], 1.5)
        draw_status(g_v.ms, "M-state:", ["missing", "generating...", "generated"], 2.0)
        draw_status(g_v.mc, "Path:", ["missing", "computing...", "computed"], 2.5)
    ...
    def draw_mission_profile(self):
        ...
        if self.is_displayed[2] == 1:
            if len(border.vertices) > 1:
                ...
                pygame.draw.aalines(surface, (200, 0, 0), closed=True, points=points)
            d_f.draw_rescaled_points(surface, border.vertices, self.selection[2], color=(200, 0, 0))
        ...
        if self.is_displayed[1] == 1:
            for obstacle_i in obstacles:
                ...
                pygame.draw.circle(surface, (200, 200, 0), obstacle_i_dash_xy, obstacle_i_dash_r)
            ...
            if self.inputing == 1 and self.input_type == 1:
                ...
                pygame.draw.circle(surface, (200, 200, 0), self.input_position, inputing_radius)

        # draw off-axis object,
        if self.is_displayed[7] == 1:
            off_obj_pos = self.dashboard_projection(off_axis_object)
            pygame.draw.circle(surface, (0, 0, 0), off_obj_pos, WAYPOINT_SIZE * 2.3, width=4)
            pygame.draw.circle(surface, (255, 255, 255), off_obj_pos, WAYPOINT_SIZE * 2, width=2)
            pygame.draw.circle(surface, (0, 0, 0), off_obj_pos, WAYPOINT_SIZE * 1.8, width=2)

        # airdrop pos
        if self.is_displayed[4] == 1:
            airdrop_object_pos = self.dashboard_projection(airdrop_object)
            pygame.draw.circle(surface, (0, 0, 0), airdrop_object_pos, WAYPOINT_SIZE * 2.3)
            pygame.draw.circle(surface, (255, 255, 255), airdrop_object_pos, WAYPOINT_SIZE * 2, width=2)
            pygame.draw.circle(surface, (255, 255, 255), airdrop_object_pos, WAYPOINT_SIZE * 1, width=2)

        # emergent obj
        if self.is_displayed[8] == 1:
            emergent_object_pos = self.dashboard_projection(emergent_object)
            pygame.draw.circle(surface, (0, 0, 0), emergent_object_pos, WAYPOINT_SIZE * 1.4)
            pygame.draw.circle(surface, (100, 255, 100), emergent_object_pos, WAYPOINT_SIZE * 1)
        ...
        if self.is_displayed[6] == 1:
            ...
            pygame.draw.rect(surface, (0, 0, 0), lost_rect, width=2)
            side = WAYPOINT_SIZE * 2
            lost_rect = pygame.Rect(lost_pos[0] - side / 2, lost_pos[1] - side / 2, side, side)
            pygame.draw.rect(surface, (255, 255, 255), lost_rect, width=2)
        ...
        if self.is_displayed[5] == 1:
            ...
            pygame.draw.circle(surface, (0, 0, 0), ugv_goal_object_pos, WAYPOINT_SIZE * 1.4)
            pygame.draw.circle(surface, (139, 0, 139), ugv_goal_object_pos, WAYPOINT_SIZE * 1)

        # draw mission waypoints on top of Final path in black
        if self.is_displayed[0] == 1:
            d_f.draw_rescaled_points(surface, mission_waypoints, self.selection[0], (0, 0, 255), 1)

    def draw_flight_profile(self):
        """Displays information of the flight profile
        ugv/plane position, time, etc"""

        surface = self.surf_flight
        
        # draw user interface info

        # draw plane info
        flight_profile = g_v.th.last_flight_profile()
        plane_obj = flight_profile.plane_obj
        x = round(plane_obj.pos[0])
        y = round(plane_obj.pos[1])
        z = round(plane_obj.z)
        d_f.draw_centered_text(surface, self.font, "plane x: " + str(x) + " ft", (1.5, 0.5))
        d_f.draw_centered_text(surface, self.font, "plane y: " + str(y) + " ft", (1.5, 1.0))
        d_f.draw_centered_text(surface, self.font, "plane z: " + str(z) + " ft", (1.5, 1.5))

        # draw next waypoint info
        next_waypoint = g_v.ms.waypoint_list[0]
        x = round(next_waypoint.pos[0])
        y = round(next_waypoint.pos[1])
        z = round(next_waypoint.z)
        d_f.draw_centered_text(surface, self.font, "way x: " + str(x) + " ft", (1.5, 2.5))
        d_f.draw_centered_text(surface, self.font, "way y: " + str(y) + " ft", (1.5, 3.0))
        d_f.draw_centered_text(surface, self.font, "way z: " + str(z) + " ft", (1.5, 3.5))

        # draw differences between the two
        d_xy = round(next_waypoint.distance_2d_to(plane_obj))
        d_z = round(next_waypoint.z - plane_obj.z)
        d_f.draw_centered_text(surface, self.font, "d_xy: " + str(d_xy) + " ft", (1.5, 4.5))
        d_f.draw_centered_text(surface, self.font, "d_z: " + str(d_z) + " ft", (1.5, 5.0))

        # draw heading of plane
        print(f'gui: {plane_obj.heading}')
        plane_direction = g_f.rotate_vector((0, 500), -plane_obj.heading)
        d_f.draw_arrow(surface, plane_obj, plane_direction, (255, 20, 147))
        ...
        for index, way in enumerate(chosen_path.waypoint_list):
            if self.mission_state_display[way.mission_index] == 1:
                ...
                if pre_way is not None:
                    ...
                    d_f.draw_curved_edge(surface, pre_way, way, color)
